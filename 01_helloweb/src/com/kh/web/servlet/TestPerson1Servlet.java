 ...
 * 
 * Servlet 생명주기
 * - Servlet은 was구동내내 딱 하나의 객체만 만들어져서 처리된다(재사용). singletone방식.
 * 
 * 1. Servlet 객체생성(기본생성자 호출). new연산자없이도 tomcat에서 알아서해준다. 	-최초client호출시 1회
 * 2. init메소드 호출  	-최초client호출시 1회
 * 3. HttpServlet의 service메소드 호출 	-client매요청마다
 * 4. 전송방식에 따라 doGet | doPost 호출	-client매요청마다
 * 5. destroy호출(tomcat종료시 객체반환)	-마지막1회
 *
 *
 * 멱등
 * - 서비스 전후로 database의 상태가 바뀌지않는 경우
 * - 멱등방식이면 (select) -> GET
 * - 멱등방식이 아니면 (insert, update, delete) -> POST
...
//			doPost(request,response);
			...
			out.println("</head>");
			out.println("<body>");
			out.println("<h1>개인취향 검사 결과 GET</h1>");
