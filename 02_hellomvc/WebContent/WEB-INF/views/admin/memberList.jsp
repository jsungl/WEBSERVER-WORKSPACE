	 ...
	 <div id="search-container">
        ...
        <div id="search-memberId" class="search-type">
            <form action="<%=request.getContextPath()%>/admin/memberFinder">
                ...
                <input type="text" name="searchKeyword" value="<%= "memberId".equals(type) ? kw : "" %>" size="25" placeholder="검색할 아이디를 입력하세요."/>
        ...
        <div id="search-memberName" class="search-type">
            <form action="<%=request.getContextPath()%>/admin/memberFinder">
                ...
                <input type="text" name="searchKeyword" value="<%= "memberName".equals(type) ? kw : "" %>" size="25" placeholder="검색할 이름을 입력하세요."/>
                <button type="submit">검색</button>			
            </form>	
        </div>
...
<script>
	...
	$(".member-role").on("change",function(){
		...
		if(confirm("[" + memberId + "] 회원의 권한을 [" + memberRoleStr + "]로 변경하시겠습니까?")){
			...
			$(this).val(preVal).prop("selected",true);
			return false;
		}
	});
